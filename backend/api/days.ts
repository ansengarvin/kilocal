  ...
  try {
    ...
    if (!req.body || !req.body.date) {
      ...
      if (result.rowCount != 0) {
        ...
        text =
          "INSERT INTO days(user_id, date) VALUES($1, $2) RETURNING id, user_id, date";
...
router.post("/:date/food", requireAuthentication, async function (req, res) {
  try {
    if (!req.body || req.body.calories === null) {
      res
        .status(400)
        .send({ err: "entry must have request body with calories" });
    } else {
...
router.get("/:date", requireAuthentication, async function (req, res) {
  try {
    ...
    let text =
      "SELECT id, name, calories, amount, carbs, fat, protein FROM foods WHERE day_id = $1";
    let values = [day_id];
    let result = await pool.query(text, values);
